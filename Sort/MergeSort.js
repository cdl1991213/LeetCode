...
export function MergeSort(arr){
    ...
    function LocalRecursiveFunction(arr, L=0, R=n-1){
        ...
        // 分别在左半部分和右半部分进行归并排序
        LocalRecursiveFunction(arr, L, midValueIndex)
        LocalRecursiveFunction(arr, midValueIndex + 1, R)
...
// 2. 归并排序
export function minSumByMergeSort(arr){
    const n = arr.length
    ...
    // 局部递归函数
    function LocalRecursiveFunction(arr, L=0, R=arr.length-1){
        // 小组只有一个元素，即小组有序
        if(L == R){
            // 小和为0
            return 0
        ...
        let leftMinSum = LocalRecursiveFunction(arr, L, midValueIndex)
        let rightMinSum = LocalRecursiveFunction(arr, midValueIndex+1, R)
    
        // 合并左右序列, 并计算当前递归层次的小和
        let currentMinSum = Merge(arr, L, midValueIndex, R)
...
// 归并排序
export function reverseSequencePairByMergeSort(arr){
    const n = arr.length
    ...
    function LocalRecursiveFunction(arr, L=0, R=arr.length-1){
        ...
        let leftReverseOrderPairQuantity = LocalRecursiveFunction(arr, L, midValueIndex)
        let rightReverseOrderPairQuantity = LocalRecursiveFunction(arr, midValueIndex+1, R)
    
        // 合并左右序列，并计算当前合并过程中找到的逆序列数量
        let currentReverseOrderPairQuantity =  Merge(arr, L, midValueIndex, R)
